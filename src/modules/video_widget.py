...
class AudioPreviewWidget(QWidget):
    """Inline audio player embeddable as a preview panel (no video track)."""
    ...
    def set_path(self, path: str) -> None:
        ...
        if not path or not os.path.exists(path):
            ...
            self._lbl_time.setText("0:00 / 0:00")
            return
        _dlog("AudioPreviewWidget.set_path", f"loading {path!r}")
    ...
    def _on_error(self, error, msg: str) -> None:
        _dlog("AudioPreviewWidget._on_error", f"{error}: {msg}")
